    ...
    public class ItemsPuntuacionConexion
    {
        ...
        public List<PuntuacionDTO> obtenerPuntuacionPorCompania(int idCompania)
        {
            ...
            using (SQLiteConnection conexion_ = new SQLiteConnection(conexion))
            {
                ...
                string query = "SELECT ip.IDItemPuntuacion, jc.IDCategoria, jc.IDJurado, p.Nombre as NombreJurado, case when pd.Puntuacion is not null then pd.Puntuacion else 0 end as Puntuacion, cc.IDCompania FROM Categoria_Puntuacion cp inner JOIN ItemsPuntuacion ip on ip.IDItemPuntuacion = cp.IDItemPuntuacion inner JOIN Jurado_Categoria jc on jc.IDCategoria = cp.IDCategoria inner JOIN Participantes p on p.IDParticipante = jc.IDJurado inner JOIN Compania_Categoria cc on cc.IDCategoria = cp.IDCategoria LEFT JOIN Puntuaciones_Detalle pd on pd.IDJurado = jc.IDJurado AND pd.IDCategoria = cp.IDCategoria AND pd.IDItemPuntuacion = ip.IDItemPuntuacion AND cc.IDCompania = pd.IDCompania WHERE cc.IDCompania = @idCompania order by jc.IDJurado, ip.IDItemPuntuacion";
